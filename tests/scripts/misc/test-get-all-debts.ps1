# Test Script for GET /api/v1/debts - List All Pending Payments
# Endpoint: GET /api/v1/debts
# Role: financial-staff
# Permission: view.debts
...
function Show-Header {
    ...
    Write-Host "  API TEST: GET /api/v1/debts - Listar Pagos Pendientes" -ForegroundColor Cyan
    Write-Host "═" * 80 -ForegroundColor Cyan
    Write-Host ""
}
...
$url = "$BaseUrl/api/v1/debts$queryString"
...
Write-Host "   X-User-Permission: view.debts" -ForegroundColor Gray
Write-Host ""
...
try {
    ...
    if ($body.data.pending_payments) {
        $pagination = $body.data.pending_payments
        ...
        Write-Host "   Total de Adeudos: $($pagination.total)" -ForegroundColor Gray
        ...
        if ($pagination.items -and $pagination.items.Count -gt 0) {
            Write-Host "💳 Detalles de Adeudos ($($pagination.items.Count) items):" -ForegroundColor Cyan
            Write-Host ""
            
            $pagination.items | ForEach-Object -Begin { $idx = 1 } {
                Write-Host "   [$idx] Estudiante: $($_.user_name)" -ForegroundColor White
                Write-Host "       Concepto: $($_.concept_name)" -ForegroundColor Gray
                Write-Host "       Monto: $(Format-Currency $_.amount)" -ForegroundColor Yellow
                Write-Host ""
                $idx++
            }
        } else {
            Write-Host "⚠️  No hay adeudos pendientes" -ForegroundColor Yellow
        }
    }
    ...
    switch ($statusCode) {
        ...
        403 {
            Write-Host "   • Usuario no tiene permisos 'view.debts'" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "1. Obtener página 1 de adeudos (15 por página):" -ForegroundColor White
Write-Host "   .\test-get-all-debts.ps1 -Token 'your-token-here'" -ForegroundColor Cyan
Write-Host ""
Write-Host "2. Buscar un estudiante específico (por email):" -ForegroundColor White
Write-Host "   .\test-get-all-debts.ps1 -Token 'your-token-here' -Search 'juan.perez@example.com'" -ForegroundColor Cyan
Write-Host ""
Write-Host "3. Buscar por CURP:" -ForegroundColor White
Write-Host "   .\test-get-all-debts.ps1 -Token 'your-token-here' -Search 'SAQ890101HDFLRR99'" -ForegroundColor Cyan
Write-Host ""
Write-Host "4. Buscar por No. Control:" -ForegroundColor White
Write-Host "   .\test-get-all-debts.ps1 -Token 'your-token-here' -Search '23001234'" -ForegroundColor Cyan
Write-Host ""
Write-Host "5. Obtener página 2 con 20 items por página:" -ForegroundColor White
Write-Host "   .\test-get-all-debts.ps1 -Token 'your-token-here' -Page 2 -PerPage 20" -ForegroundColor Cyan
Write-Host ""
Write-Host "6. Forzar actualización del caché:" -ForegroundColor White
Write-Host "   .\test-get-all-debts.ps1 -Token 'your-token-here' -ForceRefresh `$true" -ForegroundColor Cyan
Write-Host ""
Write-Host "7. Combinado: Buscar + Paginar + Forzar caché:" -ForegroundColor White
Write-Host "   .\test-get-all-debts.ps1 -Token 'your-token-here' -Search 'juan' -Page 2 -PerPage 25 -ForceRefresh `$true" -ForegroundColor Cyan
Write-Host ""
