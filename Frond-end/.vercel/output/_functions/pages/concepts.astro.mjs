import { e as createComponent, k as renderComponent, r as renderTemplate, h as addAttribute, m as maybeRenderHead } from '../chunks/astro/server_6O5fLeT-.mjs';
import 'piccolore';
...
var _a;
const $$Concepts = createComponent(($$result, $$props, $$slots) => {
  const pageTitle = "Conceptos de Cobro";
  const PRIMARY_ACCENT_COLOR = "bg-[#2E584C]";
  const PRIMARY_HOVER = "hover:bg-[#274D43]";
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": pageTitle, "data-astro-cid-dlqx7rnz": true }, { "default": ($$result2) => renderTemplate(_a || (_a = __template([" ", '<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 bg-gray-50 min-h-screen" data-astro-cid-dlqx7rnz> <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pb-4 border-b border-gray-300" data-astro-cid-dlqx7rnz> <div class="flex items-start" data-astro-cid-dlqx7rnz> <div', ' data-astro-cid-dlqx7rnz> <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" data-astro-cid-dlqx7rnz> <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" data-astro-cid-dlqx7rnz></path> </svg> </div> <div data-astro-cid-dlqx7rnz> <h1 class="text-3xl font-extrabold text-gray-800 tracking-tight" data-astro-cid-dlqx7rnz>Gesti\xF3n de Conceptos de Cobro</h1> <p class="text-md text-gray-500 mt-1" data-astro-cid-dlqx7rnz>Definici\xF3n y asignaci\xF3n de adeudos a grupos de estudiantes.</p> </div> </div> <a href="/new"', ` data-astro-cid-dlqx7rnz> <i class="fas fa-plus-circle mr-2" data-astro-cid-dlqx7rnz></i> Crear Nuevo Concepto
</a> </header> <div id="concepts-grid" class="grid gap-6" style="grid-template-columns: repeat(auto-fit, minmax(320px, 1fr));" data-astro-cid-dlqx7rnz></div> </div> <script type="module" client:load>
...
function finalizeConcept(id) {
  ...
  const confirmMsg = \`\xBFFinalizar "\${concepts[index].title}"?\\n\\nSe marcar\xE1 como INACTIVO. \xBFDeseas eliminar tambi\xE9n los cargos PENDIENTES asociados?\`;
...
/* --- RENDERIZADO CON TU DISE\xD1O ORIGINAL --- */
      ...
      <div class="bg-white rounded-xl shadow-lg border \${isInactive ? 'border-amber-200 bg-amber-50/30' : 'border-gray-200'} p-5 flex flex-col justify-between transition hover:shadow-2xl">
        ...
        <div class="space-y-3">
          <div class="hidden text-xs text-gray-600 bg-gray-50 p-2 rounded border mb-2" id="details-\${c.id}">
              \${escapeHtml(c.description || 'Sin descripci\xF3n.')}
          </div>
          ...
          <button onclick="document.getElementById('details-\${c.id}').classList.toggle('hidden')" class="w-full text-[10px] text-gray-400 hover:text-gray-600 uppercase font-bold tracking-widest transition">
            Ver descripci\xF3n <i class="fas fa-chevron-down ml-1"></i>
          </button>
        </div>
      </div>
    \`;
...
window.deleteConcept = (id) => {
  if(!confirm('\u26A0\uFE0F \xBFELIMINAR concepto?')) return;
...
<\/script>  `], [" ", '<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 bg-gray-50 min-h-screen" data-astro-cid-dlqx7rnz> <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pb-4 border-b border-gray-300" data-astro-cid-dlqx7rnz> <div class="flex items-start" data-astro-cid-dlqx7rnz> <div', ' data-astro-cid-dlqx7rnz> <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" data-astro-cid-dlqx7rnz> <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" data-astro-cid-dlqx7rnz></path> </svg> </div> <div data-astro-cid-dlqx7rnz> <h1 class="text-3xl font-extrabold text-gray-800 tracking-tight" data-astro-cid-dlqx7rnz>Gesti\xF3n de Conceptos de Cobro</h1> <p class="text-md text-gray-500 mt-1" data-astro-cid-dlqx7rnz>Definici\xF3n y asignaci\xF3n de adeudos a grupos de estudiantes.</p> </div> </div> <a href="/new"', ` data-astro-cid-dlqx7rnz> <i class="fas fa-plus-circle mr-2" data-astro-cid-dlqx7rnz></i> Crear Nuevo Concepto
</a> </header> <div id="concepts-grid" class="grid gap-6" style="grid-template-columns: repeat(auto-fit, minmax(320px, 1fr));" data-astro-cid-dlqx7rnz></div> </div> <script type="module" client:load>
const CONCEPTS_KEY = 'conceptsList';
const STUDENTS_KEY = 'studentsList';
const CHARGES_KEY = 'chargesList';
...
renderConcepts();
window.addEventListener('storage', renderConcepts);
<\/script>  `])), maybeRenderHead(), addAttribute(`p-3 ${PRIMARY_ACCENT_COLOR} rounded-lg shadow-xl mr-4 flex-shrink-0`, "class"), addAttribute(`mt-4 sm:mt-0 px-6 py-2.5 ${PRIMARY_ACCENT_COLOR} text-white font-semibold rounded-lg shadow-lg ${PRIMARY_HOVER} transition-colors duration-300 text-sm sm:text-base`, "class")) })}`;
}, "C:/Users/sarah/Documents/GitHub/CBTA/Frond-end/src/pages/concepts.astro", void 0);

const $$file = "C:/Users/sarah/Documents/GitHub/CBTA/Frond-end/src/pages/concepts.astro";
const $$url = "/concepts";
